...
class Tree
{
        ...
        if (is_array($children)) {
            foreach ($children as $child) {
                ...
                if ($maxLevel === 0 || ($maxLevel !== 0 && $maxLevel > $level)) {
                    ...
                    $mLevel = $level + 1;
                    $tmpChildren = $this->getTreeArray($child['id'], $maxLevel, $mLevel);
                    if (is_array($tmpChildren) && count($tmpChildren) > 0) {
                        $child['children'] = $tmpChildren;
                    }
                    $returnArray[] = $child;
                }
     ...
     * @return array
     */
    public function getMenuArray($myId, $maxLevel = 0, $level = 1, $path = '')
    {
        $returnArray = [];
        ...
        if (is_array($children)) {
            foreach ($children as $child) {
                $tmpArr = array(
                    'path' => $level == 1 ? '/' . $child['viewUrl'] : $child['viewUrl'],
                    'component' => $level == 1 ? 'Layout' : '@/views/' . $path . '/' . $child['viewUrl'],
                    'name' => $child['name'],
                    'level' => $level,
                    'meta' => array(
                        'title' => $child['name'],
                ...
                if ($maxLevel === 0 || ($maxLevel !== 0 && $maxLevel > $level)) {
                    ...
                    $mLevel = $level + 1;
                    $tmpChildren = $this->getMenuArray($child['id'], $maxLevel, $mLevel, $child['viewUrl']);
                    if (is_array($tmpChildren) && count($tmpChildren) > 0) {
                        $tmpArr['children'] = $tmpChildren;
                    } else {
